...
for part in ['train', 'val', 'test']:
    ...
    with open(lst_filepath, 'w') as fw:
        for i in range(num_examples):
            MAX_FG_IMAGES = 2
            num_bishop_images = np.random.randint(1, MAX_FG_IMAGES + 1)
            num_rook_images = np.random.randint(1, MAX_FG_IMAGES + 1)
            types = [0 for _ in range(num_bishop_images)]
            types += [1 for _ in range(num_rook_images)]
